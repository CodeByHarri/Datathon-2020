...
gg1 = ggplot(trends2020$data,aes(x = data.digits, fill = data.digits)) + 
  geom_bar(position = "identity", fill = "#63FF83") + 
  theme_minimal() + xlab("First Digit")# + labs(title="Benfords Law")


gg2 = ggplot(trends2020$s.o.data,aes(x = data.second.order.digits, fill = data.second.order.digits)) + 
  geom_bar(position = "identity", fill = "#63FF83") + 
  theme_minimal() + xlab("Second Order First Digit") #+ labs(title="Second Order Benfords Law")
...
subplot(ggplotly(gg1)%>%layout(annotations = a),
        ...
        nrows = 2)%>% layout(showlegend = F, yaxis = list(anchor = 'x1'), 
            yaxis2 = list(showticklabels = F), yaxis4 = list(showticklabels = F), 
            xaxis = list(showticklabels = F), xaxis2 = list(showticklabels = F)) %>%
    add_annotations(
        text = "First Digit in Load",
        x = 0.5,
        y = 0,
        yref = "paper",
        xref = "paper",
    ...
    add_annotations(
        text = "Benfords law on Electricity Load 2020 VS 2019 In Western Europe",
        x = 0.5,
        y = 1,
        yref = "paper",
        xref = "paper",
