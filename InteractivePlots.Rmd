...
vicdata$diff = vicdata$TOTALDEMAND.x - vicdata$TOTALDEMAND.y

p = plot_ly() %>%
  add_trace( data = vicdata, x = ~SETTLEMENTDATE.y, y = ~diff, type = 'scatter',
             mode = 'lines',  color ="#e63946",alpha = 0.7)

p = plotly_build(p)

p = p %>% layout(title = 'Energy Consumption in 2020 Victoria Difference with 2019 Adjusted for Seasonality',
         xaxis = list(title = 'Time'),
         yaxis = list(title = 'Energy Consumption (MW)'))
